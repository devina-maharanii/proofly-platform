...
## Phase 16 versioned skill-taxonomy architecture

The **Skills and taxonomy** module owns governed reference data: `SkillFamily`, stable `Skill` keys, `SkillDefinitionRevision`, descriptive `SkillLevel` definitions, `SkillRelation`, `TaxonomyVersion`, and `TaxonomySuggestion`. The initial software-developer vocabulary is version `1.0.0` and is limited to Foundations, Frontend, Backend, Data and infrastructure, and Product engineering. The module owns vocabulary semantics and evidence expectations; it does not own a full talent profile, a proof decision, reputation, hiring, payment, or an AI-derived skill assertion.

Every future skill reference stores the stable skill key, definition revision, and taxonomy version with its source context. `EvidenceItem`, `SubmissionVersion`, `ReviewDecision`, `Proof`, `Challenge` requirements, and rubric criteria may reference the taxonomy but remain owned by their existing modules. A contextual skill-evidence link includes evidence type, stated descriptive level where applicable, work context, difficulty rationale or rubric/challenge reference, visibility, and separately derived verification state. Search and matching consumers must receive these explicit facts and the source evidence reference; they may not infer ability from activity, convert requirement weights into a person score, or show an unverified claim as verified proof.

The released level vocabulary is **Familiar**, **Working**, **Independent**, **Advanced**, and **Reviewer**. Levels are descriptive, evidence-based, and context-specific. The `Reviewer` level is not an authorization grant and cannot substitute for the Reviewer role, reviewer-approval rules, conflict controls, or a human proof-verification decision. Relations are only parent/child, related, prerequisite, or common project context. Challenge and rubric weights are transparent, versioned requirements of that source context, not an opaque matching or candidate score.

### Governance and compatibility contract

Canonical taxonomy changes are server-governed commands in a future approved Skills module; direct browser writes are prohibited. A `TaxonomySuggestion` is a separate user input that can move from submitted to triaged, accepted, rejected, or deferred, but it never mutates canonical data. Publishing a new canonical skill, definition revision, relation, or deprecation requires the taxonomy owner, qualified software-domain review, a reason, affected key/revision list, compatibility assessment, and append-only audit record. Security/privacy/policy review is additionally required where evidence meaning, visibility, or authorization boundaries change.

Published taxonomy records are immutable. Version `1.0.0` is the initial vocabulary identifier; patch versions clarify non-semantic wording, minor versions add compatible approved vocabulary, and major versions signal a semantic or replacement change requiring consumer review. A later definition change creates a new revision and version. A deprecated skill is retained with its historic label, definition revision, version, and optional successor mapping, so published proof continues to explain what was evaluated when it was verified. No silent rename, destructive deletion, or historical proof rewrite is permitted.

Phase 16 is documentation-only. It creates no table, migration, seed, API route, server action, profile, search index, matching behavior, AI extractor, or score. The current Phase 15 RLS map remains unchanged and deny-by-default for absent skills-domain storage. When a Skills module is approved, its migration must add explicit ownership, version, visibility, audit, RLS, suggestion-write, reviewer/governance authorization, historical-readability, and adversarial compatibility tests in the same phase. Any future assistive AI use must follow `AI_GOVERNANCE.md`: source-linked, uncertainty-aware, user-correctable, and never permitted to invent skills, alter canonical taxonomy, verify proof, or make a consequential decision.
